...
<body class="min-h-screen overflow-x-hidden">
    ...
    <main class="relative z-20 pt-32 pb-20 flex flex-col items-center justify-center min-h-screen px-4">
        ...
        <div class="w-full max-w-6xl text-center relative z-10">
            ...
            <div class="quiz-card max-w-6xl mx-auto" data-aos="fade-up" data-aos-duration="1000" data-aos-delay="200">
                <div class="quiz-card-content">
                    ...
                    <h2 class="text-4xl font-bold text-white mb-4">📝 Kuis Tata Bahasa</h2>
                    <div class="bg-purple-500/20 rounded-2xl p-6 mb-8 border border-purple-400/30">
                        <h3 class="text-2xl font-bold text-white mb-2">Genre: {{ $game['genre'] }}</h3>
                    ...
                    <div class="border-t border-white/20 pt-8">
                        <form action="{{ route('grammar.submit_game', $game['game_id']) }}" method="POST" id="quiz-form">
                            @csrf
                            <div class="space-y-8">

                                @if (isset($game['sentences_to_fix']) && is_array($game['sentences_to_fix']))
                                    @foreach ($game['sentences_to_fix'] as $index => $sentence)
                                        <div class="sentence-card" data-aos="fade-up" data-aos-delay="{{ $index * 100 }}">
                                            <label class="block text-xl font-bold text-white mb-4">
                                                <span class="bg-gradient-to-r from-purple-500 to-pink-500 text-white px-4 py-2 rounded-full text-sm mr-3">
                                                    {{ $index + 1 }}
                                                </span>
                                                Kalimat {{ $index + 1 }}
                                            </label>
                                            ...
                                            <!-- Kalimat Asli -->
                                            <div class="original-sentence mb-6">
                                                <p class="text-white/90 text-lg">"{{ $sentence }}"</p>
                                            </div>

                                            <!-- Input Koreksi -->
                                            <div>
                                                <label for="sentence_{{ $index }}" class="block text-lg font-medium text-white mb-3">
                                                    ✏️ Koreksi Anda:
                                                </label>
                                                ...
                                                <p class="text-gray-300 text-sm mt-2">
                                                    Jika kalimat sudah benar, tulis ulang kalimat tersebut apa adanya.
                                                </p>
                                            </div>
                                        </div>
                                    @endforeach
                                @else
                                    <div class="bg-red-500/20 border-l-4 border-red-500 text-red-200 p-4 rounded-lg">
                                        ...
                                        <p>Gagal memuat kalimat kuis.</p>
                                    </div>
                                @endif

                                <!-- Tombol Submit -->
                                <div class="pt-8 border-t border-white/20" data-aos="fade-up" data-aos-delay="600">
                                    <button type="submit" class="quiz-button btn-glow w-full py-4 text-xl">
                                        📤 Kirim Koreksi
                                    </button>
                                    <p class="text-gray-300 mt-4 text-sm">
                                        Pastikan semua kalimat telah diperiksa sebelum mengirim!
                                    </p>
                                </div>
                            </div>
                        </form>
    ...
    <script>
        ...
        document.querySelectorAll('.form-input').forEach(input => {
            input.addEventListener('focus', function() {
                this.parentElement.classList.add('ring-2', 'ring-purple-400/50');
            });

            input.addEventListener('blur', function() {
                this.parentElement.classList.remove('ring-2', 'ring-purple-400/50');
            });
        });
    </script>
</body>
